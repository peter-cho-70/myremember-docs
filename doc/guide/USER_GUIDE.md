...
**"진행 상황"/"관련 노트"는 자동으로도 채워진다(2026-08-15부터)**: 로컬 서버
(`scripts/preview.sh`)로 쓰는 동안, 할 일 위젯에서 완료 체크한 항목과 그날 새로
만들거나 고친 areas/topics 노트가 각 섹션에 자동으로 채워진다(`scripts/sync-daily-log.py`).
직접 적은 다른 내용은 그대로 남고, 자동으로 채워진 구간만 다시 갱신된다 — 손으로 옮겨
적을 필요가 없어진다.
...
- **Gmail / 캘린더 / 관심종목 스냅샷**: `scripts/webviewer/data/dashboard-snapshot.json`이
  있으면 표시된다. **각 탭 우측 상단의 새로고침 아이콘을 누르면**(2026-08-15부터) 로컬
  서버가 `claude` CLI를 직접 호출해 Gmail/Calendar MCP 도구로 실데이터를 다시 가져와 이
  파일을 갱신하고 사이트도 재생성한다 — 별도로 Claude에게 말을 걸 필요 없이 버튼 클릭
  하나로 끝난다. 도구 호출이 여러 번 필요해 1~3분 걸릴 수 있고, 그동안 버튼이 잠기고
  아이콘이 돈다. `claude` CLI가 설치·로그인돼 있어야 하며(API 키 아님, 이 컴퓨터의
  Claude 구독 로그인을 그대로 씀), 실패하면 아이콘 아래에 이유가 표시된다. 스냅샷이
  없으면 빈 상태로 표시될 뿐 빌드가 실패하지는 않는다.
...
### 폰에서 보기 (Tailscale, 2026-08-15부터)

`myremember-vault`(비밀번호 게이트 클라우드 배포본)를 다시 빌드+푸시하지 않고도, 지금
이 컴퓨터에서 띄운 로컬 서버를 폰에서 바로 열 수 있다 — 저장하자마자 폰에도 바로
보인다. 단, **이 컴퓨터가 켜져 있고 `scripts/preview.sh`가 떠 있어야만** 동작한다(상시
서비스가 아니다).

- **최초 1회 설정**: 맥에는 `brew install --cask tailscale`로 설치 후 메뉴바 아이콘에서
  로그인(구글/깃허브 등 계정), 폰에는 앱스토어/플레이스토어에서 "Tailscale" 설치 후
  **맥과 같은 계정**으로 로그인 — 둘 다 로그인하면 같은 사설망(tailnet)에 묶인다(무료
  플랜으로 충분).
- **쓸 때마다**: `scripts/preview.sh`로 서버를 켜면 Tailscale 로그인이 돼 있을 경우
  터미널에 `Tailscale로 폰 등 다른 기기에서: http://100.x.y.z:4500` 줄이 자동으로 하나 더
  뜬다. 그 주소를 폰 브라우저에 입력(또는 홈 화면에 추가)하면 된다 — 같은 Wi-Fi가
  아니어도, 데이터망이어도 접속된다. 이 IP는 `tailscale ip -4` 명령으로도 확인할 수 있다.
- **보안**: 일반 Wi-Fi의 다른 기기나 인터넷 전체에는 여전히 안 열린다 — 같은 tailnet에
  로그인된 기기만 닿을 수 있어서, 지금까지의 "127.0.0.1 전용" 원칙이 그대로 유지된다.
- Tailscale 자체(설치·CLI·MagicDNS·Exit Node 등)에 대한 더 자세한 내용은
  `topics/tailscale-사용자-매뉴얼.md` 노트 참고.

### 사용법 페이지 (`guide.html`)

사이드바 "사용법" 탭에 이 문서(9절까지)의 핵심 내용을 화면 안에서 바로 볼 수 있게 요약해
뒀다 — 터미널로 이 파일을 열지 않아도 웹뷰어 안에서 바로 참고할 수 있다. 배포된 사이트
(읽기 전용)에서는 쓰기 기능 관련 안내가 자동으로 빠진다.
