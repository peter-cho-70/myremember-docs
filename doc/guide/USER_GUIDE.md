...
## CADENCE 회고 (Phase 0 검증, docs/cadence-concept.html §13)
- Q1. 오늘 가장 오래 걸린 일과 왜 오래 걸렸는지:
- Q2. 오늘 막힌 지점 (대기/정보부족/도구한계/사람·조율/집중안됨/범위불명확/없었음):
- Q3. 오늘 에너지 (1~5):
- Q4. 오늘 알게 된 것 한 줄 (없으면 비워두기):
```

마지막 섹션(CADENCE 회고)은 2026-08-14부터 추가됐다 — 하루 3분 정도로 4문항만 채우는
가벼운 습관 기록으로, 나중에 별도 앱/자동 수집 없이 이 daily note만으로 "내가 정말 이걸
꾸준히 채우는 사람인지"부터 검증해보는 용도다. 부담되면 비워둬도 무방하다.
...
- **할 일**: 날짜별 체크리스트. 직접 추가/완료 토글/수정/삭제가 가능하고 캘린더 연동은 없다.
  ...
  (`scripts/webviewer/data/todos.json`)로도 미러링된다. **"할 일을 스마트폰에
  보내기" 버튼**(2026-08-14 추가, 로컬 서버에서만 보임)을 누르면 그 시점 상태를
  클라우드 배포 사이트(`myremember-vault`)에 바로 반영한다 — 노트가 안 바뀐 날에도
  전체 사이트를 다시 만들 필요 없이 이 버튼 하나로 끝나고, `MYREMEMBER_CLOUD_PASSWORD`도
  필요 없다(할 일 데이터 파일 하나만 갱신). 폰에서 새로고침하면 그 시점 PC 내용이
  우선 반영되며, 그 사이 폰에서 직접 체크/수정한 내용은 덮어써질 수 있다(뷰어보다는
  "PC 상태를 폰으로 보내는" 성격에 가까움 — 폰에서 PC로 되돌아오진 않는 건 그대로다).
